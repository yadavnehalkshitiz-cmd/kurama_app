...
def get_custom_folder(user_id: int) -> str | None:
    """Return custom folder for a chat, or None."""
    cfg = _configs.get(str(user_id), {})
    return cfg.get("custom_folder")


def set_custom_folder(user_id: int, folder: str):
    cid = str(user_id)
...
def reset_custom_folder(user_id: int):
    cid = str(user_id)
    if cid in _configs:
...
def get_queue(user_id: int) -> list:
    return _configs.get(str(user_id), {}).get("queue", [])


def set_queue(user_id: int, queue: list):
    cid = str(user_id)
    if cid not in _configs:
        _configs[cid] = {}
...
def add_to_queue(user_id: int, url: str):
    q = get_queue(user_id)
    q.append(url)
    set_queue(user_id, q)


def pop_queue(user_id: int) -> str | None:
    q = get_queue(user_id)
    if not q:
        return None
    url = q.pop(0)
...
def _ensure_user_profile(user_id: int) -> dict:
...
def get_user_credits(user_id: int) -> int:
    ...
    cfg = _ensure_user_profile(user_id)
...
def consume_download_credit(user_id: int) -> tuple[bool, bool]:
...
def refund_credit(user_id: int) -> int:
...
def update_payment_status(user_id: int, tx_id: str, status: str) -> bool:
    ...
    for cid, cfg in _configs.items():
        for p in cfg.get("pending_payments", []):
            if p.get("status") == "PENDING":
                pending.append({"user_id": cid, **p})
    return pending


def admin_set_credits(user_id: int, credits: int):
...
def admin_revoke_subscription(user_id: int):
