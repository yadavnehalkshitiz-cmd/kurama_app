...
class BillingTests(unittest.TestCase):
    ...
    def test_admin_summaries_use_app_user_id(self):
        self.user_config.admin_set_credits(41, 7)
        self.user_config.add_pending_payment(41, "TX-USER")

        users = self.user_config.get_all_users()
        pending = self.user_config.get_pending_payments()

        self.assertEqual(users[0]["user_id"], "41")
        self.assertNotIn("chat_id", users[0])
        self.assertEqual(pending[0]["user_id"], "41")
        self.assertNotIn("chat_id", pending[0])
